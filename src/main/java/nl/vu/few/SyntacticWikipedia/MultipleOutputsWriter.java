...
/**
 * The <code>MultipleOutputsWriter</code> action writes its input to multiple files,
 * by means of a so-called file-writer which creates a folder for each tuple key. 
 * The all folder and files are placed in a user-specified output directory.
 */
public class MultipleOutputsWriter extends Action {

	final static Logger log = LoggerFactory.getLogger(MultipleOutputsWriter.class);
	...
	private void openFile(ActionContext context, String folder) throws IOException {
		...
		File dir = new File(outputDirectory+"/"+fileType);
		...
		if (customWriter != null) {
			try {
				file = Class.forName(customWriter).asSubclass(FileWriter.class).newInstance();
			} catch (Throwable e) {
	...
	public void process(Tuple inputTuple, ActionContext context,
		...
		++count;
		String type = ((TString)inputTuple.get(0)).getValue();
		if (file == null || fileType.equals(type)) {
			openFile(context, type);
		}
